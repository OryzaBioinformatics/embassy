 ...
 /* printf("mirnatog=%d\n",mirnatog); */
  ...
  for (j=TURN+2;j<=n; j++) {
	...
	for (k=i+1; k<=eMIN(i+MAXLOOP+1,j-TURN-2); k++) {
	  u1 = k-i-1;
	  for (l=eMAX(k+TURN+1,j-1-MAXLOOP+u1); l<j; l++) {
...
PRIVATE void scale_pf_params(unsigned int length)
{
  ...
  for (i=0; i<5; i++) {
    GT = F_ninio37[i]*TT;
    for (j=0; j<=MAXLOOP; j++)
      expninio[i][j]=exp(-eMIN(MAX_NINIO,j*GT)*10/kT);
  }
...
PUBLIC struct ConcEnt *get_concentrations(double FcAB, double FcAA, double FcBB, double FEA, double FEB, double *startconc, AjPFile outf)
{
