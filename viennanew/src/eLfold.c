...
PUBLIC float  Lfold(char *string, char *structure, int maxdist,AjPFile outf);
...
PRIVATE int fill_arrays(char *sequence, int maxdist, AjPFile outf);
...
float Lfold(char *string, char *structure, int maxdist, AjPFile outf) {
  ...
  energy = fill_arrays(string, maxdist, outf);
...
PRIVATE int fill_arrays(char *string, int maxdist, AjPFile outf) {
...
PRIVATE char * backtrack(char *string, int start, int maxdist) {
  ...
  while (s>0) {
    int ml, fij, cij=0, traced, i1, j1, d3, d5, mm, p, q, jj=0;
