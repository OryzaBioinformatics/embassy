...
/* 
	...
	For MEME, the following characters are hashed to other characters if the
	alphabet is DNA0 or PROTEIN0:
		PROTEIN			DNA
		=======			======
		B to D (DN to D)	B to C (GTC to C)
					D to G (GAT to G)
					H to A (ACT to A)
					K to T (GT to T)
					M to A (AC to A)
					...
					R to A (GA to A)
					S to G (GC to G)
		U to C (selenoC to C)	U to T (uridine to T)
					V to G (GCA to G)
					W to T (AT to T)
		X to C (any to C)
					Y to C (TC to C)
...
)
{
  ...
  */
  if (!strcmp(alphabet, DNA0)) {
    ...
    hash('W') = hash('T');
    hash('Y') = hash('C');
  } else if (!strcmp(alphabet, PROTEIN0)) {
    /* PROTEIN */
    hash('B') = hash('D');
    hash('U') = hash('C');
    hash('X') = hash('C');
...
/*
	...
	Find out which standard alphabet it is a subset of, extend
	it to the standard alphabet and create a permutation matrix
...
)
{
  ...
  char *min_alph;			/* required letters */
  ...
  */
  if (strspn(old_alph, DNAB) == old_alen) {
    ...
    min_alph = DNA0;		/* required letters */
    ...
    min_alph = PROTEIN0;	/* required letters */
...
/*
        ...
        Convert a DNAB sequence in place to its inverse complement.
*/
/**********************************************************************/
...
)
{
  ...
  */
  ...
  for (i=0,s=sequence,h=hash_seq; i<len; i++,s++,h++) *h = chash(xlate_dna, s);
...
/* $Header: /packages/emboss/MASTER/emboss/emboss/embassy/meme/src/hash_alph.c,v 1.1.1.1 2001/12/24 09:05:38 ableasby Exp $ */
