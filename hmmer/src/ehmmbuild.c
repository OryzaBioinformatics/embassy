...
static void position_average_score(struct plan7_s *hmm, char **seq, float *wgt,
				   ...
				   float *pernode,float *ret_avg);
static float frag_trace_score(struct plan7_s *hmm, char *dsq,
			      struct p7trace_s *tr, float *pernode,
			      float expected);
...
int main(int argc, char **argv) 
{
    ...
    enum p7_weight
    {		      		/* weighting strategy */
	WGT_NONE, WGT_GSC, WGT_BLOSUM, WGT_VORONOI, WGT_ME
    } w_strategy;
    enum p7_config
    ...
    ajNamInit("emboss");
    ajAcdInitP("ehmmbuild",argc,argv,"HMMER");
	...
	c_strategy = P7_HAND_CONSTRUCTION;
	...
	DetermineAlphabet(aseq, ainfo.nseq);

    /* Set up Dirichlet priors */
    if (prifile == NULL)
	pri = P7DefaultPrior();
    else
	pri = P7ReadPrior(prifile);
	...
	P7ReadNullModel(rndfile, randomseq, &p1);
	...
	/* don't do effective seq #; it's calibrated for protein */
	do_eff     = FALSE;
    ...
    if      (cfg_strategy == P7_BASE_CONFIG)
	puts("Global alignment (hmms)");
    ...
    else if (cfg_strategy == P7_LS_CONFIG)
	puts("Multiple domain (hmmls)");
    ...
    if (c_strategy == P7_HAND_CONSTRUCTION)
	puts("Manual, from #=RF annotation");
    else if (c_strategy==P7_FAST_CONSTRUCTION)
	printf("Fast/ad hoc (gapmax %.2f)\n", gapmax);
    else
	printf("MAP (gapmax hint: %.2f)\n", gapmax);
    ...
    if      (w_strategy == WGT_NONE)
	puts("none");
    else if (w_strategy == WGT_GSC)
	puts("G/S/C tree weights");
    else if (w_strategy == WGT_BLOSUM)
	printf("BLOSUM filter at %.2f id\n", blosumlevel);
    else if (w_strategy == WGT_VORONOI)
	puts("Sibbald/Argos Voronoi");
    else if (w_strategy == WGT_ME)
	puts("Maximum entropy");
    printf(
	 "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n");
     ...
     */
    if (w_strategy == WGT_GSC || w_strategy == WGT_BLOSUM ||
	w_strategy == WGT_VORONOI)
    {
	...
	GSCWeights(aseq, &ainfo);
    else if (w_strategy == WGT_BLOSUM)
	BlosumWeights(aseq, &ainfo, blosumlevel);
    else if (w_strategy == WGT_VORONOI)
	VoronoiWeights(aseq, &ainfo); 
    ...
    else if (ainfo.name != NULL)
	Plan7SetName(hmm, ainfo.name);
    else
    {
     ...
     */
    if (ainfo.acc  != NULL)
	Plan7SetAccession(hmm, ainfo.acc);
    if (ainfo.desc != NULL)
	Plan7SetDescription(hmm, ainfo.desc);
    ...
    default:              ajFatal("bogus configuration choice");
    }
    printf("done.\n");
	...
	fflush(stdout);
	P7Traces2Alignment(dsq, ainfo.sqinfo, ainfo.wgt, ainfo.nseq, hmm->M,
			   tr, FALSE, &new_aseq, &new_ainfo);
	if ((fp = fopen(align_ofile, "w")) == NULL)
	{
	    ajWarn("Failed to open alignment resave file %s; using stdout "
		   "instead", align_ofile);
	    fp = stdout;
	}
		       ...
		       int do_binary)
{
	...
	{ 
	    ...
	    if (hmmfp == NULL)
	    {
		ajWarn("%s not an HMM file; can't append to it; using stdout "
		       "instead",
		     hmmfile);
		fp = stdout;
	...
	{
	    ajWarn("hey, where'd your HMM file go? Using stdout instead.");
	    fp = stdout;
	    puts("");		/* do a newline before stdout HMM starts */
	} 
    } 
	...
	{
	    ajWarn("Failed to open HMM save file %s; using stdout instead",
		   hmmfile);
	    fp = stdout;
	    puts("");		/* do a newline before stdout HMM starts */
	}
 ...
 * Return:   (void)                         
 */
static void print_all_scores(FILE *fp, struct plan7_s *hmm, AINFO *ainfo,
			     char **dsq, int nseq, struct p7trace_s **tr)
{
    int idx;				/* counter for sequences */
...
static void save_countvectors(char *cfile, struct plan7_s *hmm)
{
	...
	ajFatal("failed to open count vector file %s for writing", cfile);
 ...
 */
static void position_average_score(struct plan7_s    *hmm, 
				   char             **dsq, 
				   float             *wgt,
				   int                nseq,
				   struct p7trace_s **tr,
				   ...
				   float             *ret_avg)
{
	...
	for (tpos = 0; tpos < tr[idx]->tlen; tpos++)
	{
		...
		pernode[k] += wgt[idx] * 
		    Scorify(TransitionScoreLookup(hmm,
						  tr[idx]->statetype[tpos],
						  tr[idx]->nodeidx[tpos],
						  tr[idx]->statetype[tpos+1],
						  tr[idx]->nodeidx[tpos+1]));
	}
 ...
 */
static void maximum_entropy(struct plan7_s *hmm, char **dsq, AINFO *ainfo,
			    int nseq, float eff_nseq, struct p7prior_s *prior,
			    struct p7trace_s **tr)
{
	...
	while (new_entropy > relative_entropy && ++i2 < max_iter)
	    ...
	    Plan7SWConfig(hmm, 0.5, 0.5);
	    P7Logoddsify(hmm, TRUE);
	    position_average_score(hmm, dsq, new_wgt, nseq, tr, pernode,
				   &expscore);
	    for (idx = 0; idx < nseq; idx++) 
		sc[idx]      = frag_trace_score(hmm, dsq[idx], tr[idx],
						pernode, expscore);
    ...
    if (i1 == max_iter)
	fprintf(stderr,"   -- exceeded maximum iterations; giving up --\n");
