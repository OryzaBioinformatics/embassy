 ...
 *
 * RCS $Id: ehmmsearch.c,v 1.1.1.1 2001/12/24 09:05:38 ableasby Exp $
...
static struct workpool_s *workpool_start(struct plan7_s *hmm, AjPSeqall seqall, 
...
static void record_domains(struct tophit_s *h, 
			   ...
			   int do_null2);
static void main_loop_serial(AjPFile outf,struct plan7_s *hmm, AjPSeqall seqall, 
...
#ifdef HMMER_PVM
static void main_loop_pvm(AjPFile outf,struct plan7_s *hmm, AjPSeqall seqall, 
			  float globT, double globE, int Z, int do_forward,
...
int main(int argc, char **argv) 
{
...
#ifndef HMMER_PVM
    if (do_pvm)
	ajFatal("PVM support is not compiled into your HMMER software; -pvm "
		"doesn't work.");
#endif
    ...
    if (! do_pvm)
	main_loop_serial(outf, hmm, seqall, globT, globE, Z, do_forward,
			 do_null2, do_xnu, num_threads, histogram, ghit,
			 dhit, &nseq);
#ifdef HMMER_PVM
    else
	main_loop_pvm(outf, hmm, seqall, globT, globE, Z, do_forward,
		      do_null2, do_xnu, histogram, ghit, dhit, &nseq);
#endif
    ...
    if (!Z)
	Z = nseq;		/* set Z for good now that we're done. */
    ...
    if (hmm->flags & PLAN7_STATS)
	ajFmtPrintF(outf,"  [HMM has been calibrated; E-values are "
		    "empirical estimates]\n");
    else
	ajFmtPrintF(outf,"  [No calibration for HMM; E-values are upper "
		    "bounds]\n");
    ...
    ajFmtPrintF(outf,"%-*s %7s %5s %5s    %5s %5s    %7s %8s\n",
		namewidth, "Sequence", "Domain ", "seq-f", "seq-t", "hmm-f",
		"hmm-t", "score", "E-value");
    ajFmtPrintF(outf,"%-*s %7s %5s %5s    %5s %5s    %7s %8s\n",
		namewidth, "--------", "-------", "-----", "-----", "-----",
		"-----", "-----", "-------");
	...
	if (motherp * (double) Z >= globE || mothersc <= globT) 
	    ...
	    ajFmtPrintF(outf,"%-*s %3d/%-3d %5d %5d %c%c %5d %5d %c%c %7.1f "
			"%8.2g\n",
			namewidth, name, 
			domidx, ndom,
			sqfrom, sqto, 
	...
	{
	    if (i > 0)
		ajFmtPrintF(outf,"\t[no more scores below domE threshold]\n");
	    break;
	}
	else if (sc <= domT)
	{
	    if (i > 0)
		ajFmtPrintF(outf,"\t[no more scores above domT threshold]\n");
	    break;
	}
    }
    if (i == 0)
	...
	for (i = 0; i < dhit->num; i++)
	{
	    ...
	    {
		if (i > 0)
		    ajFmtPrintF(outf,"\t[no more alignments below domE "
				"threshold\n");
		break;
	    }
	    else if (sc <= domT)
	    {
		if (i > 0)
		    ajFmtPrintF(outf,"\t[no more alignments above domT "
				"threshold\n");
		break;
	    }
	}
	if (i == 0)
	    ajFmtPrintF(outf,"\t[no hits above thresholds\n");
	if (i == Alimit)
	    ajFmtPrintF(outf,"\t[output cut off at A = %d top alignments]\n",
			Alimit);
    }
 ...
 * Returns:  (void)
 */
static void main_loop_serial(AjPFile outf, struct plan7_s *hmm,
			     AjPSeqall seqall,  float globT, double globE,
			     int Z, int do_forward, int do_null2, int do_xnu,
			     int num_threads, struct histogram_s *histogram, 
			     struct tophit_s *ghit, struct tophit_s *dhit,
			     int *ret_nseq)
{
#ifdef HMMER_THREADS
...
#ifdef HMMER_THREADS
    wpool = workpool_start(hmm, seqall, do_xnu, do_forward, do_null2, globT,
			   globE, Z, ghit, dhit, histogram, num_threads);
    workpool_stop(wpool);
    nseq = wpool->nseq;
    workpool_free(wpool);
	...
	if (sc > globT && evalue < globE) 
	{
	    ...
	    record_domains(dhit, hmm, dsq, sqinfo.name, sqinfo.desc,
			   sqinfo.len, tr, pvalue, sc, do_null2); 
	}
			  ...
			  int *ret_nseq)
{
	...
	desclist[nseq] = (sqinfo.flags & SQINFO_DESC) ?
	    Strdup(sqinfo.desc) : NULL;
	...
	if (sent_trace)
	{
	    ...
	    record_domains(dhit, hmm, dsqlist[slaveidx], 
			   namelist[slaveidx], desclist[slaveidx],
			   lenlist[slaveidx],
	...
	desclist[slaveidx] = (sqinfo.flags & SQINFO_DESC) ?
	    Strdup(sqinfo.desc) : NULL;
	...
	if (sent_trace)
	{
	    ...
	    record_domains(dhit, hmm, dsqlist[slaveidx], 
			   namelist[slaveidx], desclist[slaveidx],
			   lenlist[slaveidx], tr, pvalue, sc, do_null2); 
	    P7FreeTrace(tr);
	}
	AddToHistogram(histogram, sc);
	...
	/* free seq info */
	free(namelist[slaveidx]);
	if (desclist[slaveidx] != NULL)
	    free(desclist[slaveidx]);
	free(dsqlist[slaveidx]);
...
void *worker_thread(void *ptr)
{
	...
	if (sqinfo.len == 0)
	    continue;	/* silent skip of len=0 seqs (wormpep!?!) */
	...
	/* 3. Save the output in tophits and histogram structures,
	...
	evalue = wpool->Z ?
	    (double) wpool->Z * pvalue : (double) wpool->nseq * pvalue;
