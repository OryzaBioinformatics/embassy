...
GSCWeights(char **aseq, AINFO *ainfo)
{
  ...
  struct phylo_s *tree;
  float  *lwt=NULL, *rwt=NULL;  /* weight on left, right of this tree node */
  float  *fwt=NULL;             /* final weight assigned to this node */
  int      i;
...
VoronoiWeights(char **aseq, AINFO *ainfo)
{
  ...
  int     *nsym=NULL;           /* # syms seen in each column      */
  ...
  int     iteration;           
  int     best=0;			/* index of nearest real sequence  */
		...
		char ***ret_anew, int *ret_nnew, AINFO **ret_newinfo)
{
  ...
  for (idx = 0; idx < nnew; idx++)
    SeqinfoCopy(&(newinfo->sqinfo[idx]), &(ainfo->sqinfo[list[idx]]));
				/* copy optional info */
  if (ainfo->rf != NULL) newinfo->rf = Strdup(ainfo->rf);
  if (ainfo->cs != NULL) newinfo->cs = Strdup(ainfo->cs);
